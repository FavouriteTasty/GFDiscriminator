...
class vEMDiffuseTrainingDataset2Dto3D(data.Dataset):  # Dataset for vEMDiffuse training using the whole isotropic volume
    def __init__(self, data_root, data_len=-1, norm=True, percent=False, phase='train', image_size=[256, 256],
                 method='vEMDiffuse-i',
                 z_times=6, loader=pil_loader_noL):
        self.data_root = data_root
    ...
    def __getitem__(self, index):
        ...
        # simulate slide accroding to beer lambert law
        img_up = np.prod([np.array(gts[i]).astype(np.float32)/256 for i in range(len(gts))], axis=0)
        img_up = img_up / img_up.max()
        img_up = Image.fromarray((img_up * 255).astype(np.uint8))
        img_below = img_up.copy()
        img_up, img_below, gts = self.aug(img_up, img_below, gts)
        img_up = self.tfs(img_up)
        img_below = self.tfs(img_below)
        out_gts = []
